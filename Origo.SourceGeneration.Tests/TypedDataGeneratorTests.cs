...
public class TypedDataGeneratorTests
{
    ...
    [Fact]
    public void Home_GeneratedPublicMembers_CarryXmlSummaries()
    {
        var text = RunHome(_homePrimitivesAttribute).AllGeneratedText;

        // TypedData.TryGet* methods and the explicit conversion operators are
        // public API generated by the source generator; IDE tooltips depend on
        // these summaries just like handwritten public members (AGENTS §1.7).
        Assert.Contains(
            "/// <summary>Reads the stored value as <c>int</c> when this instance uses the matching registered kind.</summary>",
            text);
        Assert.Contains(
            "/// <summary>Creates a <see cref=\"TypedData\"/> instance from a <c>int</c> value.</summary>",
            text);
