...
- **`ISndEntity.OwningSession` is now automatically bound at entity creation** — entities created through `ISndSceneHost.CreateEntity` (and by extension `ISessionRun.Spawn` / `RecoverFromMetaList` / load paths) in both `FullMemorySndSceneHost` (Core) and `GodotSndManager` (GodotAdapter) now have their `OwningSession` property bound to the session that owns the scene host. Previously `BindSession` was defined but never called, making `OwningSession` always throw. Strategies can now reliably use `entity.OwningSession` to reach their owning session.
- **Ambient session mechanism removed** — `PushAmbientSession`, the `_ambientSessions` stack, and `AmbientSessionScope` are deleted from `SndContext`. `CurrentSession` now always returns the foreground session. The `ISessionScopedSceneHost` interface is deleted; replaced by the narrower `IOwningSessionBindable` (only `SetOwningSession`). All ambient push calls in `SessionManager` (6 sites) and `SndRuntime.SpawnCore`/`SpawnManyCore` (2 sites) are removed. Strategy hooks should use `entity.OwningSession` (now automatically bound) to resolve their owning session instead of relying on the ambient stack.
- **BREAKING: `SndWorld.CreateEntity` is now `internal`** — the entity factory takes a per-scene-host observer topology and is no longer part of the public API surface. Entities are still created through `ISndSceneHost` / `SndRuntime` exactly as before; observer-strategy behavior (mount/unmount, cross-entity teardown, and save/load of observer bindings) and the save-file format are unchanged. Observer bindings are now tracked by a per-scene-host topology rather than per-entity, which is an internal architecture change with no effect on observer-strategy authoring.
- **BREAKING: `ISndSessionAccess` and `ISndEntityOperations` removed from `ISndContext`** — the two role interfaces are deleted. `ISessionManager SessionManager` is now declared directly on `ISndContext` (the only session-related member remaining at the top-level interface). `CurrentSession`, `IsFrontSession`, `RequestKillAll`, and `RequestKillEntity` are no longer part of the public `ISndContext` contract. Strategies that used `ctx.CurrentSession` should migrate to `entity.OwningSession` (if the entity is available) or cast to `SndContext` for framework-internal access to the ambient session. `ctx.RequestKillEntity(name)` callers should use `entity.OwningSession.RequestKillEntity(name)` or `ctx.SessionManager.ForegroundSession!.RequestKillEntity(name)`.
- **BREAKING: `ISndContext.CurrentSession` / `IsFrontSession` now reflect the session currently being processed, not always the foreground session** — strategy lifecycle hooks (Process / AfterSpawn / AfterLoad / BeforeSave / BeforeQuit) always observe their owning session, including entities spawned directly into a background session's scene host. `CurrentSession` is kept on the concrete `SndContext` class (not on the `ISndContext` interface) for internal use by the ambient session mechanism. When no session is being processed, it falls back to the foreground session.
...
- **BREAKING: `ISessionRun.SceneHost` removed from the public interface** — session-level scene host access is no longer exposed to strategies. Strategies that previously accessed `ctx.CurrentSession.SceneHost.FindByName(name)` should instead call `entity.OwningSession.FindByName(name)` (same session) or `entity.OwningSession.SessionManager.TryGet(key)` (cross-session). `ISessionRun` now provides direct entity operation methods: `FindByName`, `GetEntities`, `Spawn`, `SpawnMany`, and `RequestKillEntity` — these replace the corresponding `SceneHost.*` calls. The concrete `SessionRun.SceneHost` remains accessible as `internal` for framework-internal use only.
- **BREAKING: `ISndEntity` now requires `OwningSession`** — every entity must have a non-null `OwningSession` property bound at creation time. Strategies can use `entity.OwningSession` to reach their owning session instead of going through the global `ISndContext.CurrentSession` door. Implementations of `ISndEntity` (including test stubs and adapter-level entities) must provide this property.
...
- **TypedData generator kind-space diagnostics `ORIGOSG003` / `ORIGOSG004`** — the source generator fails the build when a registered type's kind falls outside the byte range `[1, 255]` (`ORIGOSG003`, including the case where an out-of-range kind would otherwise wrap onto an in-use value) or when overlapping `SndInlineTypes` startKind ranges assign one kind byte to multiple types (`ORIGOSG004`). This prevents silent kind collisions that would corrupt TypedData storage at runtime.
- **`ISndEntity.OwningSession`** — every entity exposes its owning `ISessionRun` via this non-null property. The session is bound at creation time; accessing it before binding throws `InvalidOperationException`. This allows strategies to reach their session (and other sessions via `OwningSession.SessionManager`) without relying on the global `ISndContext.CurrentSession` lookup.
...
- **`ISessionRun` entity operations** — `FindByName(name)`, `GetEntities()`, `Spawn(meta)`, `SpawnMany(metaList)`, `RequestKillEntity(name)`. These methods provide a session-scoped entity operation facade, replacing the now-internal `SceneHost` property. `Spawn`/`SpawnMany` include name-uniqueness validation (via `SndRuntime`) and fire `AfterSpawn` lifecycle hooks; `CreateEntity` remains available on `ISndSceneHost` for framework-internal use without hooks.
...
- **Active-strategy recovery no longer silently drops non-ActiveStrategyBase strategies** — loading an entity whose active-strategy slot references a strategy that is not an `ActiveStrategyBase` now throws `InvalidOperationException`, and any active strategies already acquired during the same recovery are rolled back so no half-initialized state remains. Previously such a mismatch was silently discarded, producing an entity missing an active strategy with no signal. This aligns active-strategy recovery with the fail-fast behavior already enforced for lifecycle-strategy recovery, surfacing save/code mismatches immediately.
...
- **TypedData generator silent degradation removed** — eliminated source-generator code paths that could emit `TypedData` accessors silently returning `0` / `default` for out-of-bounds inline registrations (a latent data-corruption risk). Such registrations are now rejected at build time via `ORIGOSG001` / `ORIGOSG002`. Generated output for the existing Origo.Core and Origo.GodotAdapter registrations is unchanged.
