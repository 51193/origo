...
- **`Origo.SourceGeneration.Tests`** — dedicated test project that drives the TypedData source generator over in-memory compilations, verifying home/adapter output, the inline-vs-`_ref` storage model, the new diagnostics, and generation determinism (Coverlet line coverage gate ≥ 85%). It also includes a lenient performance benchmark comparing the generated inline `TypedData` (write / read / mixed dispatch) against an unoptimized boxing implementation — asserted (within a generous slowdown bound and a per-benchmark time cap) and printed as a comparison table on every CI run.
