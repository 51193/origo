...
- **`Origo.Core` and `Origo.Core.Kernel` dependency direction** — `Origo.Core` references Kernel with `PrivateAssets="compile"` so restore supplies kernel runtime assets without kernel compile assets. `Origo.ConsoleBridge` is already shell-only and references the Core shell package; `Origo.GodotAdapter` keeps private kernel compile access behind its single shell package.
