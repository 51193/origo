...
- **Stable custom console command registration** — `IOrigoRuntime.RegisterConsoleCommandHandler` exposes the documented tooling extension path for `IConsoleCommandHandler` implementations. Core and adapter hosts register through the same shell contract, registration fails fast when either console channel is missing, and adapter built-in handlers now use the same path.
...
- **Shell API baseline gate** — `scripts/api-inventory.sh` builds the shell projects in Release, runs the tracked Roslyn inventory in `tools/ApiInventoryTool`, and verifies `shell-api-baseline.json`; unapproved shell API additions, removals, or signature changes fail through `scripts/ci.sh`, the normal CI matrix, and the Release workflow until the baseline is updated in the same reviewed change. Previous-package validation runs from `ORIGO_PREVIOUS_API_BASELINE` once a released baseline exists; the first 0.1.0 release states that behavior explicitly.
- **Matt Pocock engineering skills integration** — GitHub Issues in `51193/origo` and local Markdown counterparts under `.scratch/issues/` share issue numbers; five default triage states and the existing bilingual domain-documentation route are configured in `docs/agents/`, with `AGENTS.md` retaining authority.
- **Core shell/kernel package split** — `Origo.Core.Contracts` now carries the stable consumer surface (interfaces, pure data, metadata, strategy base classes, data-source contracts, logging abstractions, and shared pure helpers), `Origo.Core.Kernel` carries the runtime/SND/save/data-source/console implementations and internal `Origo.Core.Kernel.Ports`, and `Origo.Core` is the consumer shell package. The shell host facade `OrigoHost` / `OrigoHostOptions` and the stable `IOrigoRuntime` / `ISndWorldAccess` interfaces let a consumer compile and run a Core workflow while kernel compile assets stay out of the consumer restore graph.
...
- **Weekly snapshot build workflow** — the Monday 02:30 UTC scheduled run publishes a `-nightly.YYYYMMDD` build only when the week that just ended ([previous Monday 00:00, current Monday 00:00) UTC) contains new commits, ignoring its own version-bump commits so idle weeks publish nothing; manual dispatch additionally covers the current partial week, and only `main` can run it. A publish derives the base version from `<Version>` (so `0.0.10` to `0.0.11` moves tag, package version, and assembly metadata together), bumps `<Version>` plus `AssemblyVersion` / `FileVersion` to that base, commits all three stamps to `main`, tags that commit as `v<base>-nightly.YYYYMMDD`, and then starts the Release workflow explicitly with `gh workflow run release.yml -f tag=<tag>`: a tag pushed with `GITHUB_TOKEN` does not fire the `push` trigger and `workflow_dispatch` is the documented exception, so the formal pipeline is reused without storing a personal access token. Automatic snapshots are published as pre-releases (the weekly job asks for one), so a snapshot never takes over as the repository's Latest release, while a hand-pushed tag keeps the release state chosen on GitHub.
...
- **BREAKING: Core implementation moved behind the shell/kernel boundary** — concrete runtime, SND, persistence/storage, data-source codec/factory, console routing, and state-machine implementation types now compile into `Origo.Core.Kernel` instead of `Origo.Core`. Consumers that referenced concrete `OrigoRuntime`, `SndWorld`, `SndContext`, `SndContextParameters`, or `SndEntity` must migrate to `IOrigoRuntime`, `ISndWorldAccess`, `ISndContext`, and the other Contracts interfaces; `OrigoHost` is the supported shell creation path.
- **BREAKING: Godot adapter bootstrap uses the Core kernel port and a stable runtime surface** — `OrigoAutoHost` now creates its runtime, system blackboard, console channels, observer topology, and SND context through `Origo.Core.Kernel.Ports.AdapterHostKernelPort`; its public `Runtime` is `IOrigoRuntime`, `SndManager` is implementation-internal, `OrigoDefaultEntry.ConfigureStrategies` receives `ISndWorldAccess`, and `CommandHandlerBase` holds `IOrigoRuntime`. Adapter code that referenced concrete kernel types through the entry classes must switch to those stable interfaces.
- **BREAKING: shared stable types moved to `Origo.Core.Contracts`** — strategy base classes and attributes, `GridPos`, `Blackboard`, `Logger<T>`, `LogMessageBuilder`, `NullLogger`, `TypeStringMapping`, `DataSourceConverterRegistry`, and `PathUtility` now live in the Contracts assembly. Source namespaces are unchanged, but compiled assembly identities changed and old binaries must be recompiled.
- **BREAKING: Godot adapter bridge and manager types are internal** — `GodotSndManager` and `GodotSndEntity` (including their Godot-generated nested signal types) are implementation details of the single `Origo.GodotAdapter` shell package. The supported entry surface remains `OrigoAutoHost`, `OrigoDefaultEntry`, `SndEntityNodeExtensions`, and the other documented adapter shell types; consumers that referenced the manager or entity classes directly must use those entries and the stable Core contracts.
- **Core shell/kernel and ConsoleBridge dependency direction** — `Origo.Core` references Kernel with `PrivateAssets="compile"` so restore supplies kernel runtime assets without kernel compile assets. `Origo.ConsoleBridge` references `Origo.Core.Contracts` directly, so a bridge-only consumer receives the console/logging contracts without Core shell, Kernel runtime, or Godot assets; `Origo.GodotAdapter` keeps private kernel compile access behind its single shell package.
- **Release workflow syncs version stamps from the tag and writes the release commit back to `main` and the tag** — a `v*` tag push now rewrites `Directory.Build.props` so `<Version>` equals the tag version and `AssemblyVersion` / `FileVersion` use its four-part numeric form, creates a `chore(release)` commit when anything changed, and runs validation, tests, benchmarks, and packing on that commit. Only after all of them succeed does the workflow issue one atomic push that fast-forwards `main` and moves the tag to the release commit, with lease checks on both refs, before creating the GitHub Release; validation or packing failures leave every ref untouched. The write-back requires the tag to be created on the current main tip and fails if `main` or the tag moved. The later formal-metadata checks (`CHANGELOG` version block, empty `[Unreleased]`, analyzer release tracking, and `docs/README.*` version stamps) still require a properly prepared tag.
...
- **Formal release metadata verification checks the version stamps** — `scripts/verify-release.sh` now fails when `Directory.Build.props` `<Version>` differs from `TAG_VERSION`, or when `AssemblyVersion` / `FileVersion` are not the four-part numeric form of that version. The pre-tag checklist therefore catches a stale or hand-edited version stamp before the tag is created; snapshot releases continue to skip formal metadata checks. `scripts/test-verify-release.sh` runs from `scripts/lint-scripts.sh` to pin the pass, mismatch, and snapshot-skip behavior.
