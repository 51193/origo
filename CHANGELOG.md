...
- **BREAKING:** `DataSourceNode.AsByte/AsSByte/AsShort/AsUShort/AsInt/AsUInt/AsLong/AsULong/AsFloat/AsDouble/AsDecimal/AsBool` (12 methods) replaced by a single unified `As<T>()` generic method. `AsString()` and `AsChar()` are retained as standalone methods.
