...
- **BREAKING: `ISndContext.CurrentSession` / `IsFrontSession` now reflect the session currently being processed, not always the foreground session** — strategy lifecycle hooks (Process / AfterSpawn / AfterLoad / BeforeSave / BeforeQuit) always observe their owning session, including entities spawned directly into a background session's scene host. When no session is being processed, `CurrentSession` falls back to the foreground session; in particular, **deferred actions flushed at end-of-frame observe the foreground session** rather than the session active when the action was enqueued. The internal per-session context wrapper (`SessionSndContext`) was removed in favor of this ambient model.
