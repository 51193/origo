...
public class BackgroundSessionTests
{
    ...
    public void OwnEntities_IsolatedFromForeground()
    {
        ...
        Assert.Null(ctx.SessionManager.ForegroundSession?.FindByName("bg_entity"));
    ...
    private sealed class SessionContextSpyStrategy : LifecycleStrategyBase
    {
        ...
        public override void Process(ISndEntity entity, double delta, ISndContext ctx)
        {
            if (((SndContext)ctx).CurrentSession is not null)
                _seen.Value?.Add(((SndContext)ctx).CurrentSession.LevelId);
        }
    }
}
