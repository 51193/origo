...
public class BackgroundSessionTests
{
    ...
    public void SharedSndWorld_StrategiesFireInBackground()
    {
        ...
        using var bg = ctx.SessionManager.CreateBackgroundSession("bg", "bg");
        var entity = bg.SceneHost.Spawn(CreateMetaWithStrategy("bg_entity"));
    ...
    public void FullWorkflow_CreatePopulateTickSave()
    {
        ...
        var guard01 = host.Spawn(CreateMetaWithIndices("guard_01", TrackingStrategyIndex, ProcessStrategyIndex));
        if (guard01 is IEntityLifecycle lc1)
            lc1.FireAfterSpawnHooks();
        var guard02 = host.Spawn(CreateMetaWithIndices("guard_02", TrackingStrategyIndex));
