...
此外，Origo.GodotAdapter 通过 `[assembly: SndInlineTypes(startKind: 128, ...)]` 向 TypedData 多层内联系统注册这 14 种类型，使运行时 `GetKindForType(typeof(Vector3))` 返回确定的 kind 值（130），避免走 `is T` 运行时模式匹配的兜底路径。详见 [Origo.SourceGeneration](../../Origo.SourceGeneration/README.zh.md)。
