...
`SchedulingAndTypeMappingTests.cs` 同时承载 ActionScheduler 与 TypeStringMapping 两种能力的测试：本文档仅记录其 TypeStringMapping 相关方法（`TypeStringMapping_HasDefaultTypes_AndSupportsCustomRegistration`）；其 ActionScheduler 方法记录于 [Runtime-Core.md](Runtime-Core.md)，本文档不重复收录。
...
| `SchedulingAndTypeMappingTests.cs` | TypeStringMapping 默认类型与自定义注册（ActionScheduler 方法见 [Runtime-Core.md](Runtime-Core.md)） |
