...
|------|-------------------|
| `Planning/PlanExecutionStrategyBaseTests.cs` | Complete plan lifecycle: start/advance/complete/fail, Action strategy mount/unmount, subscription leak prevention (verifies consecutive same steps reset the ActionKey suffix; StartIntent failure rolls back wiring for retry; action completion without an active intent throws and cleans up) |
| `Planning/PlanExecutionStrategyBridgeTests.cs` | Bridge entities (adapter entities wrapping an inner SndEntity, e.g. GodotSndEntity): static LifecycleIndices reuse, step advancement with action cleanup, ActionKey cleared on termination, idempotent termination after a failed action add |
