...
| `DeadClientAfterEstablishedConnection_IsClosed_NextClientConnectsAndReplaysBacklog` | 连接已建立（flush 成功）后客户端停止读取，后续输出写失败（OnConsoleOutput 发送超时）→ 死连接被关闭、槽位释放，下一个客户端可接入并收到缓冲回放（验证：输出写失败会关闭死连接并释放槽位） | ConsoleBridge: 发送超时 detach |
