...
|-----------|-----------|---------|
| [Codec](Codec/README.en.md) | Format codecs | `JsonDataSourceCodec` (lazy expansion) + `MapDataSourceCodec` (key:value, strict fail-fast) + `RawStringDataSourceCodec` (`.sha`/`.write_in_progress` raw text) |
...
| `MemoryFileSystem.cs` | In-memory file system implementing `IFileSystem` (public, reused for tests and adapter layer) |
