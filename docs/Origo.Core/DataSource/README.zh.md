...
- **备选方向：统一树形命名空间（暂缓）**：`DataSourceNode` 已具备树形结构和 codec 即插即用两个基础，可进一步把本地文件系统、存档目录和网络资源统一挂载为树根，用 `path -> to -> file -> entity -> health_point` 式的路径导航替代多套文件 API；受限子树可结构性地表达访问范围。当前同步读取模型对本地文件足够，但远端节点会阻塞帧，且内容/元数据边界、活树写回语义都要重新定义，因此暂缓。完整权衡与重新评估信号见 [#44](https://github.com/51193/origo/issues/44)
