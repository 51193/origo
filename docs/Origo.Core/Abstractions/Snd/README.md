...
> **已删除**：`ISndSessionAccess.cs` 和 `ISndEntityOperations.cs`。`SessionManager` 现直接声明在 `ISndContext` 上。`CurrentSession`、`IsFrontSession` 保留在具体 `SndContext` 类上供框架内部使用（ambient 机制）。`RequestKillAll`/`RequestKillEntity` 已移除——实体销毁经 `entity.OwningSession.RequestKillEntity(name)` 或 `ISessionRun.RequestKillEntity(name)` 执行。
