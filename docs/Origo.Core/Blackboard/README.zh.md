...
- **SetValue**：通过 `TypedDataFactory<T>.Create(value)` 创建值类型实例存入字典，泛型类型在编译期捕获；未注册引用类型的 null 会抛 `ArgumentNullException`（已注册引用 kind 如 `string` 的 null 仍合法）
...
### 为什么未注册引用类型的 null 直接拒绝

`TypedData` 只能为已注册 kind 或非 null 引用保留类型信息；未注册引用类型的 null 会把类型退化为 `object`，写入后既无法按原类型命中，也无法可靠序列化。与其存入一个不可用条目，不如在 `SetValue` 边界立即抛 `ArgumentNullException`（fail-fast）。
