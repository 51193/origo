...
- **异常处理**：单个动作异常时记录 Error 日志后重新抛出（let-it-crash 语义），**同批次剩余动作随之丢弃**（含前序动作新入队的动作）；`OrigoRuntime.FlushEndOfFrameDeferred` 中业务队列抛异常时，当帧的 `KillPendingAllSessions` 与系统队列执行被顺延到下一帧（fail-fast 的级联效应）
