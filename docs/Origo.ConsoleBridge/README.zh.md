...
- **输出路径同步 + 发送超时**：`OnConsoleOutput` 回调中的 `StreamWriter.WriteLine` 保持同步——控制台输出是短 kernel 调用，正常消费下不会阻塞。为防慢客户端/停止读取的客户端填满 TCP 发送缓冲导致游戏帧线程僵死，连接建立时设置**有界发送超时**（`ConsoleBridgeOptions.OutputSendTimeoutMs`，默认 100ms）：单次发送阻塞超过超时即断开该连接——**detach 会真正关闭客户端连接**（释放单连接槽位，否则死连接会永久占用它，缓冲行永远等不到"下次连接"），未送达的行保留在缓冲中等待下次连接回放。**积压回放（连接建立时的历史行重放）运行在有界时间预算内**：慢速但持续读取的客户端使每行写阻塞低于发送超时、累计却可能长达数秒（期间帧线程在写锁上等待），回放在预算处中止（预算 = 4×发送超时，钳制在 [200ms, 1000ms]），剩余行保留在缓冲；单行写失败（发送超时）则结束连接。
...
- `Dispose()` 先取消活动读取，用一次 loopback 连接唤醒挂起的 accept，等待 accept 循环退出后再关闭监听 socket——teardown 竞争窗口内已被接受的连接会被归还并关闭，而不会作为半开连接被丢弃
