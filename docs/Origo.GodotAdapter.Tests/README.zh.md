...
为缩小排除面，`GodotSndManager` 的实体集合编排逻辑（增删、查找、批量恢复回滚、帧处理、击杀标记）已提取到纯 C# 的 `SndEntityCollection<T>`（`Origo.GodotAdapter/Snd/SndEntityCollection.cs`），由 `SndEntityCollectionTests` 全覆盖（98.7%）；生成代码 `TypedData.g.cs` 的 14 个 Godot 类型访问器由 `GodotTypedDataGeneratedCoverageTests` 逐类型覆盖。门禁统计范围内行覆盖率 ≥ 94%（`ThresholdStat=total`）。
