...
| 能力 | 文档 | 文件数 | 测试数 | 验证重点 |
|------|------|-------|-------|---------|
| 架构守卫 | [Architecture.md](Architecture.zh.md) | 1 | 9 | SndContext 公共角色接口完整性、会话创建/销毁、CommandHandlerBase 公共可见性、GodotSndEntity 生命周期 internal 守卫、GodotSndManager 写路径封闭、私有字段 `_camelCase` 命名、shell API 分类完整性 |
| SND 实体 | [Snd.md](Snd.zh.md) | 4 | 27 | SndEntityCollection 全能力与批量恢复回滚、程序集加载触发 Kind 注册、节点扩展契约、GodotPackedSceneNodeFactory 构造守卫 |
