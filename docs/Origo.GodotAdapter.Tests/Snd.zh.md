...
| 策略类 | 定义位置 | 用途 |
|--------|---------|------|
| `SndEntityCollectionTests` 内嵌假实体 | `SndEntityCollectionTests.cs` | 实现 `ISndEntityFacade` 的纯 C# 假实体（无 Godot 依赖），记录 `RecoverForLifecycle`/`DetachFromManager`/`RollbackAcquiredResources` 调用 |
