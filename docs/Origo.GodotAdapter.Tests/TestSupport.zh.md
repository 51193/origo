...
通用测试替身（`TestLogger`、`PerfReporter`、`TestMemoryFileSystem`、`TestSndSceneHost`（含 `DummySndEntity`）等）
已提取到共享项目 [Origo.TestSupport](../Origo.TestSupport/README.zh.md)，GodotAdapter.Tests 通过项目引用复用。
...
| `TestSndSceneHost` | `ISndSceneHost` 实现 | 内存场景宿主，维护实体字典，支持按名称查找、`AddEntity`、`RemoveEntity` 与 `RequestKillEntity`（重复 kill 抛异常）。**GodotAdapter 专用**，与共享版本行为有差异（`CreateEntity` 抛异常、含 `AddEntity` 注入方法） |
| `InMemorySndEntity` | `ISndEntity` 实现 | 内存实体替身，基于字典存取 `SetData`/`GetData`/`TryGetData`，策略/节点/观察者方法为空操作。**GodotAdapter 专用**，`InvokeStrategy` 返回 null（共享 `DummySndEntity` 抛异常） |
