...
6. **AGENTS.md 元指令变更** → 在本文档中同步引用新规则（如 AGENTS.md §1.7 注释语言要求与 vendor 源码豁免、§1.8 git 历史考量、§1.9 依赖更新分组规则——版本耦合的包族必须整体升级，不得单独修改、§3 红测试先行规则——修复缺陷必须先写真实路径复现的红测试并验证转绿，修改/扩展文件前查看其 git 历史）
