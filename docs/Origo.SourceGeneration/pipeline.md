...
> [↑ 回到 Origo.SourceGeneration](README.md) ·
> [↔ 基准数据](../benchmarks/baseline.md) ·
> [↔ TypedData 文档](../Origo.Core/Snd/Metadata/README.md)

## 概述

本文档是对 TypedData 源码生成器的**为什么、怎么做的、为什么有效**的系统性解析。文档从头梳理问题→方案→链路→性能数据→边界限制的完整推理链，供后续维护与扩展研发参考。

**目标读者**：需要在 Origo 框架上进一步扩展 TypedData 能力、适配新运行时、或理解性能权衡细节的开发者。阅读本文前，建议先通读 [Origo.SourceGeneration README](README.md) 了解双模式架构和生成内容概览。
...
这是生成方案唯一显著劣势的场景。**但它不是真实热路径**：

- 框架内部热/温路径（数据变更信号处理、加载校验、实体观察）一律用零装箱 `TryGetXxx`
- `.Data` 仅用于序列化和冷路径——这些路径固有需要 `object`，内置装箱无法避免
- **即使是最坏情况也不会在热路径出现**——下游调用方接受并理解这一取舍
...
> 数据来自 [benchmarks/baseline.md](../benchmarks/baseline.md)，采样环境：AMD Ryzen 7 9700X / .NET 10.0.9
...
- 唯一输项（`.Data` 迭代 0.14x）是冷路径中被刻意接受的取舍——框架约定热路径用 `TryGetXxx`。
...
| 文档 | 内容 |
|------|------|
| [Origo.SourceGeneration README](README.md) | 双模式架构、生成内容清单、注册机制、设计决策 |
| [TypedData 文档](../Origo.Core/Snd/Metadata/README.md) | TypedData 结构体、访问方式、推荐用法 |
| [性能基线](../benchmarks/baseline.md) | 全部基准数据、方法学、效度局限 |
| [Origo.Core.Tests / Benchmarks](../Origo.Core.Tests/Benchmarks.md) | 真实模拟基准说明 |
