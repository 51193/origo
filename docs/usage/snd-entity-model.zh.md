...
| 钩子 | 触发时机 | 典型用途 |
|------|---------|---------|
| `AfterSpawn(entity, ctx)` | 新实体生成后（由 `SndEntityFactory.Spawn` / `SpawnMany` 统一触发） | 初始化属性（hp, position 等） |
...
- [会话模型](session-model.zh.md) — 前台/后台会话
- [持久化流程](persistence-flow.zh.md) — 存档/读档
- [策略测试](strategy-testing.zh.md) — StrategyTestScenario
