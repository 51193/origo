...
> [↑ 回到 Abstractions](../README.zh.md) · [↔ 实现: Origo.Core.Contracts/Logging](../../../Origo.Core.Contracts/Logging/README.zh.md)
...
Format 和 interpolation 是调用方的关注点。Core 层的 `LogMessageBuilder`（见 [Logging 实现](../../../Origo.Core.Contracts/Logging/README.zh.md)）已提供结构化消息构建能力，`ILogger` 保持接收纯字符串的最小接口。
